...
from kite.util import (
    generate_file as create_file,
    generate_kite as gen_kite,
    date,
    send_email as email,
    logger,
)
...
def find_crossovers(dates, closing_prices):
    ...
    custom_col_1_value = closing_prices.rolling(int(custom_col_1[4:])).mean()  # SMA9
    ...
    if (
        old_closing_price > old_sma and new_closing_price <= new_sma
    ):  # Price crosses above SMA9 (crossover from below)
        crossover.append(
            (
                old_date,
                new_date,
        ...
        old_closing_price < old_sma and new_closing_price >= new_sma
    ):  # Price crosses below SMA9 (crossover from above)
        crossover.append(
            (
                old_date,
                new_date,
...
def plot_crossovers(dates, closing_prices, crossover):
    ...
    log.debug(
        f"In plot_crossover 1. Decision maker= {decision_maker}, 2. Crossover Decision= {crossover[5]}"
    )
    ...
    if crossover[5] == "Buy" and decision_maker in ("", "Buy"):
        ...
        log.info("Plotted for Buy Signal")
        decision_maker = "Sell"
        ...
        log.info("Plotted for Sell Signal")
        decision_maker = "Buy"
        trade_entry_price = crossover[3]
        stop_loss_level = trade_entry_price + stop_loss

    log.debug(
        f"After plotting 1. Decision maker= {decision_maker}, 2. Trade Entry Price= {trade_entry_price},"
        f" 3.Stop Loss Level= {stop_loss_level}"
    )

    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.title(
        f"{instrument_token} - Closing Price and Soft Margin Average with Crossovers"
    )
    plt.legend()
    plt.show()
...
def check_stop_loss(date_index, closing_price):
    ...
    if closing_price < stop_loss_level:
        ...
        log.info(
            f"Stop Loss Level Crossed. Had to make a decision on below dataset."
            f" Current closing price is = {closing_price} and stop loss level is= {stop_loss_level}"
        )
        gmail_message = (
            f"StopLoss level crossed. Check the below data and if you find any problem then take an"
...
def main():
    try:
        ...
        latest_data = [
            (dict1["date"], dict1["close"]) for dict1 in historical_data[-no_of_data:]
        ]
        latest_data.append(
            (current_india_time, current_data["NSE:NIFTY 50"]["last_price"])
        )
        ...
        if len_crossover == 1:
            ...
            email.send_gmail(
                subject="AlgoTrading - New Crossover ALERT!!", message=gmail_message
            )
            log.info(f"Gmail sent for {crossover[3]}, closing value")
        ...
        log.debug("Stop loss check is done.")
...
if __name__ == "__main__":
    log.debug("Running as standalone program.")
    while True:
        main()
        log.warning("Now Program will take 14.6 mins Pause..")
        time.sleep(14.6 * 60)
